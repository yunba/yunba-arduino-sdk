...
void extMessageArrived(MQTT::ExtMessageData& extMessageData) {
   MQTT::ExtMessage &message = extMessageData.message;
  ...
  extMessageReceived(message.cmd, message.status, String(payload), (unsigned int)message.payloadlen);
}
...
boolean MQTTClient::begin(const char * _hostname, int _port, Client& _client) {
  ...
  this->client->setDefaultExtMessageHandler(extMessageArrived);
...
void MQTTClient::publish2ToAlias(const char *alias, const char *payload, const char  *opt_json ) {
  client->publish2ToAlias(alias, (char *)payload, strlen(payload), opt_json);
}

void MQTTClient::publish2(const char *topic, const char *payload, const char  *opt_json ) {
  client->publish2(topic, (char *)payload, strlen(payload), opt_json);
}

void MQTTClient::getAlias(const char * parm) {
    client->getAlias(parm);
}
