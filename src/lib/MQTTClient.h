...
struct ExtMessageData
{
    ExtMessageData(struct ExtMessage &aMessage)  : message(aMessage)
    { }
...
public:
    typedef void (*messageHandler)(MessageData&);
	 typedef void (*extMessageHandler)(ExtMessageData&);
    ...
    void setDefaultExtMessageHandler(extMessageHandler emh)
    {
        defaultExtMessageHandler.attach(emh);
    }
     ...
     *  @return success code -
     */
    int publish(const char* topicName, void* payload, size_t payloadlen, uint64_t& id, enum QoS qos = QOS1, bool retained = false);
    ...
    int getAlias(const char *param);
	
    int publish2ToAlias(const char* alias, void* payload, size_t payloadlen, const char *opt_json);
    int publish2(const char* topic, void* payload, size_t payloadlen, const char *opt_json);
...
private:
    ...
    int deliverExtMessage(ExtMessage& message);
    bool isTopicMatched(char* topicFilter, MQTTString& topicName);
    int MQTTExtendedCmd(EXTED_CMD cmd, void *payload, int payload_len, int qos, unsigned char retained);
    ...
    FP<void, ExtMessageData&> defaultExtMessageHandler;
    ...
    #endif
    uint64_t incomingQoS2messages[MAX_INCOMING_QOS2_MESSAGES];
    bool isQoS2msgidFree(uint64_t id);
    bool useQoS2msgid(uint64_t id);
	void freeQoS2msgid(uint64_t id);
#endif
...
bool MQTT::Client<Network, Timer, a, b>::isQoS2msgidFree(uint64_t id)
{
    for (int i = 0; i < MAX_INCOMING_QOS2_MESSAGES; ++i)
    {
...
template<class Network, class Timer, int a, int b>
bool MQTT::Client<Network, Timer, a, b>::useQoS2msgid(uint64_t id)
{
    for (int i = 0; i < MAX_INCOMING_QOS2_MESSAGES; ++i)
    {
...
template<class Network, class Timer, int a, int b>
void MQTT::Client<Network, Timer, a, b>::freeQoS2msgid(uint64_t id)
{
    for (int i = 0; i < MAX_INCOMING_QOS2_MESSAGES; ++i)
    {
        if (incomingQoS2messages[i] == id)
...
template<class Network, class Timer, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::MQTTExtendedCmd(EXTED_CMD cmd, void *payload, int payload_len, int qos, unsigned char retained)
{
   int rc = FAILURE;
    ...
    len = MQTTSerialize_extendedcmd(sendbuf, MAX_MQTT_PACKET_SIZE, 0, qos, retained, id,
...
#if 1
    if (waitfor(EXTCMD, timer) == EXTCMD) {
    	rc = SUCCESS;
    }
#endif
exit:
...
template<class Network, class Timer, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::getAlias(const char *parm)
{
    return MQTTExtendedCmd(GET_ALIAS, (void *)parm, strlen(parm), QOS1, 0);
}

template<class Network, class Timer, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::publish2ToAlias(const char* alias, void* payload, size_t payloadlen, const char  *opt_json)
{
	char buf[150];

	sprintf(buf, ",yta/%s", alias);
	return publish2(buf, payload, payloadlen, opt_json);
}

template<class Network, class Timer, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::publish2(const char* topicName, void* payload, size_t payloadlen, const char  *opt_json)
{
	uint8_t *p;
	uint8_t pub_buf[256];
	uint16_t len, i = 0;
	...
	*p++ = (uint8_t)(len & 0xff);
	...
	p += len;
      ...
      JsonObject& opt = jsonBuf.parseObject(opt_json);
	if (opt.success()) {
		const char *ttl =  opt["time_to_live"];
		const char * qos = opt["qos"];
		const char *apn = opt["apn_json"];

		if (ttl != NULL) {
			len = strlen(ttl);
			*p++ = (uint8_t)PUBLISH2_TLV_TTL;
			*p++ = (uint8_t)((len >> 8) & 0xff);
			*p++ = (uint8_t)(len & 0xff);
			memcpy(p, ttl, len);
			p += len;
	...
	rc = MQTTExtendedCmd(PUBLISH2, (void *)pub_buf, (int)(p-pub_buf), /*DEFAULT_QOS*/QOS1, 0);
...
template<class Network, class Timer, int a, int MAX_MESSAGE_HANDLERS>
int MQTT::Client<Network, Timer, a, MAX_MESSAGE_HANDLERS>::deliverExtMessage(ExtMessage &message)
{
    int rc = FAILURE;
    if (defaultExtMessageHandler.attached())
    {
       // MessageData md(topicName, message);
       // defaultMessageHandler(md);
       ExtMessageData md(message);
       defaultExtMessageHandler(md);
        rc = SUCCESS;
    }
	...
	case EXTCMD:
		{
            ...
            if (MQTTDeserialize_extendedcmd(
            		(unsigned char*)&ext_msg.dup,
            		(int*)&ext_msg.qos,
            		(unsigned char*)&ext_msg.retained,
            		(uint64_t*)&ext_msg.id,
            		&ext_msg.cmd,
            		&ext_msg.status,
            		(void**)&ext_msg.payload,
            		(int*)&ext_msg.payloadlen,
            		readbuf,
            		MAX_MQTT_PACKET_SIZE) != 1)
            	goto exit;

            deliverExtMessage(ext_msg);
		}
		break;
    }
    keepalive();
...
template<class Network, class Timer, int MAX_MQTT_PACKET_SIZE, int b>
int MQTT::Client<Network, Timer, MAX_MQTT_PACKET_SIZE, b>::publish(const char* topicName, void* payload, size_t payloadlen, uint64_t& id, enum QoS qos, bool retained)
{
    int rc = FAILURE;
    Timer timer = Timer(command_timeout_ms);
...
{
    uint64_t id = 0;  // dummy - not used for anything
